...
> Tes IQ yang sah mengukur *kemampuan kognitif* melalui soal dengan **satu jawaban benar objektif**, dinilai berdasarkan performa aktual, bukan preferensi yang dilaporkan sendiri. Spesifikasi ini mendefinisikan metodologi inti sesuai standar seperti Raven's Progressive Matrices, WAIS, dan CFIT, dengan arsitektur sistem (Go/Gin/PostgreSQL) yang mendukung.
...
### 2.1 Format & Pendekatan

Tes ini menggunakan **pilihan ganda A/B/C/D bergambar** dengan **satu jawaban benar objektif**. Penilaian didasarkan pada **jumlah jawaban benar (performa aktual)**, bukan preferensi subjektif. Output berupa skor performa kognitif yang dikonversi menjadi estimasi IQ.
...
`time_taken_ms` dicatat per soal (bukan untuk skor utama, tapi untuk analitik dan deteksi anomali — lihat Bagian 9).

---

## 6. SCORING ALGORITHM

### 6.1 Pipeline Skoring
...
> **Catatan:** Skor kemampuan kognitif tidak boleh dipetakan ke konstruk kepribadian seperti narsisme, machiavellianism, atau psikopati — klaim semacam itu tidak memiliki dasar psikometri dan berisiko menyesatkan serta merugikan pengguna. Instrumen kepribadian harus diimplementasikan sebagai **tes terpisah** dengan instrumen tervalidasi (mis. Big Five/HEXACO).
...
|----------|------------------|--------|
| Timer keras 2 menit/soal | Auto-advance saat habis | ✅ Active |
...
| Randomisasi urutan opsi | Posisi A/B/C/D diacak per sesi | ✅ Active |
...
Tabel pendukung: `users`, `test_sessions`, `payments`, `admins`.

### 10.2 Catatan Keamanan Skema

| Field | Aturan |
|-------|--------|
| `correct_option` | **WAJIB** ada di database, **DILARANG** dikirim ke client dalam bentuk apa pun |
| `questions.p_value` | Nullable — diisi setelah kalibrasi dari data uji coba |
| `questions.discrimination` | Nullable — diisi setelah kalibrasi dari data uji coba |
| `iq_results.estimated_iq` | Nullable — diisi hanya setelah data normatif tersedia |
| `session_responses.selected_option` | Nullable — NULL jika terjadi timeout |
...
|--------|----------|-----------|
| `GET` | `/quiz` | Halaman tes (mengambil 20 soal) |
| `POST` | `/submit-tes` | Submit semua jawaban untuk diproses |
| `GET` | `/paywall/:id` | Halaman pembayaran untuk membuka hasil |
| `GET` | `/hasil/:id` | Halaman hasil lengkap (setelah bayar) |
...
### 12.1 Quiz Page — Spesifikasi

| Elemen | Spesifikasi |
|--------|-------------|
| Format soal | **Gambar soal + 4 opsi jawaban bergambar (A/B/C/D)** |
| Timer | **Countdown 2 menit per soal, wajib tampil** dengan warning visual |
| Navigasi mundur | **Dihapus** (integritas timing) |
| Progress bar | Ada, ditambah indikator waktu tersisa |

### 12.2 Alpine.js State
...
Struktur halaman landing, paywall, hasil, dan admin mengikuti pola standar. Halaman hasil menggunakan komponen skor domain baru (Bagian 8) dan disclaimer kejujuran skor IQ (Bagian 7.1).
...
*End of IQTEST.md v2.0 — Specification*